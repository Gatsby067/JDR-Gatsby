...
if len(st.session_state.messages) > 20:
    résumé = client.chat.completions.create(
        model="gpt-4o",
        messages=st.session_state.messages[:-10] + [{"role": "user", "content": "Résume les événements précédents en notant : les dépenses, les nouveaux objets, les rencontres, les actions effectué, les lieux visités, les détails important pour la suite"}]
